...
class CalculateComplexityTest(unittest.TestCase):
    ...
    def test_small_simple_change(self):
        files = [
            FileStats(filename="app.py", additions=5, deletions=2, language="Python")
        ]
    ...
    def test_large_multi_language_change(self):
        files = [
            FileStats(
                filename="app.py", additions=300, deletions=100, language="Python"
            ),
            FileStats(filename="main.rs", additions=200, deletions=50, language="Rust"),
            FileStats(
                filename="index.ts", additions=150, deletions=80, language="TypeScript"
            ),
            FileStats(
                filename="handler.go", additions=100, deletions=30, language="Go"
            ),
            FileStats(
                filename="App.tsx",
                additions=50,
    ...
    def test_test_heavy_change_is_lower(self):
        ...
        prod_files = [
            FileStats(filename="app.py", additions=100, deletions=50, language="Python")
        ]
        test_files = [
            FileStats(
                filename="app.py", additions=100, deletions=50, language="Python"
            ),
            FileStats(
...
class IdentifyRiskFactorsTest(unittest.TestCase):
    def test_large_pr_flagged(self):
        files = [
            FileStats(
                filename="big.py", additions=300, deletions=200, language="Python"
            )
        ]
    ...
    def test_no_tests_flagged(self):
        files = [
            FileStats(filename="app.py", additions=40, deletions=20, language="Python")
        ]
        risks = pr_analyzer.identify_risk_factors(files)
    ...
    def test_security_sensitive_file(self):
        ...
        risks = pr_analyzer.identify_risk_factors(files)
        self.assertTrue(any("Safety-sensitive" in r for r in risks))

    def test_extract_safety_file(self):
        files = [
            FileStats(
                filename="src/archivey/extract.py",
                additions=20,
                deletions=0,
                language="Python",
            )
    ...
    def test_test_substring_files_still_flag_no_tests(self):
        ...
        files = [
            FileStats(
                filename="latest_report.py",
                additions=40,
                deletions=20,
                language="Python",
            ),
            FileStats(
                filename="contest_utils.py",
                additions=30,
                deletions=10,
                language="Python",
            ),
...
class GenerateSuggestionsTest(unittest.TestCase):
    def test_returns_list(self):
        files = [
            FileStats(filename="app.py", additions=10, deletions=5, language="Python")
        ]
    ...
    def test_large_pr_split_suggestion(self):
        files = [
            FileStats(
                filename="big.py", additions=600, deletions=300, language="Python"
            )
        ]
    ...
    def test_python_suggestion(self):
        ...
        result = pr_analyzer.generate_suggestions(files, 0.1, [])
        self.assertTrue(
            any("extract path" in s.lower() or "ruff" in s.lower() for s in result)
        )
