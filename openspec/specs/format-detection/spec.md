...
The system SHALL raise `FormatDetectionError` if the stream is shorter than 32 774 bytes when ISO detection is attempted. If the stream is long enough but the magic `43 44 30 30 31` ("CD001") is not found at offset 32 769, the system SHALL return a result indicating no ISO match (falling back to extension or raising `FormatDetectionError` as appropriate).
...
#### Scenario: stream too short for ISO detection

- **WHEN** ISO detection is attempted on a stream shorter than 32 774 bytes
- **THEN** `detect_format()` raises `FormatDetectionError`
...
The system SHALL ensure that the bytes read during detection remain available to the backend that subsequently opens the archive.

- For **seekable streams**: the stream position SHALL be restored to its original position (via `seek(0)`) after detection.
- For **non-seekable streams**: the system SHALL wrap the stream in a `PeekableStream` that buffers the first `DETECTION_LIMIT` bytes and replays them transparently on subsequent reads.
...
- After format detection, presents itself as a `BinaryIO`-compatible object to the backend. Reads drain from the buffer first, then fall through to the underlying stream once the buffer is exhausted.
- `PeekableStream` is constructed only when the source is non-seekable. The backend receives the `PeekableStream` object and does not need to know whether the original source was seekable.
...
#### Scenario: non-seekable stream is wrapped

- **WHEN** `detect_format()` is called with a non-seekable `BinaryIO` (e.g. a socket or pipe)
- **THEN** the system wraps it in a `PeekableStream`
- **AND** the backend reads the peeked bytes first from the buffer, then continues reading from the underlying stream
- **AND** no bytes are silently dropped
