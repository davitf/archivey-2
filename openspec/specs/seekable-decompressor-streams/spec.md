...
2. When rapidgzip reaches EOF having delivered **no** decompressed bytes without
   raising — fully switch the stream to the stdlib gzip-window
   `DecompressorStream` (**before** returning empty success), so truncation is
   signaled and any recoverable prefix is streamed on bounded `read(n)`;
   `read()` / `readall` raise without returning bytes (same contract as
   accelerator-off). A valid empty gzip SHALL still succeed with zero bytes.
   After the switch, `tell` / `seek` / `seekable` SHALL reflect the stdlib
...
| Truncated gzip through rapidgzip (path); soft-empty EOF | stdlib `DecompressorStream` fallback → `TruncatedError` (bounded `read(n)` may stream a correct prefix first; `read(-1)` raises without returning); never silent empty success |
