...
- [x] 1.1 Clone the DEV repository into a scratch location outside the project
      tree: `git clone https://github.com/davitf/archivey-dev.git /tmp/archivey-dev`.
- [x] 1.2 Check out a pinned, reproducible commit SHA rather than tracking the
      ...
      chosen deliberately.
- [x] 1.3 Record the exact source commit SHA in the Phase 1 commit message / PR
      description for traceability.
- [x] 1.4 If the clone fails, retry over HTTPS (with backoff); only if it still
      fails, stop and surface the blocker — every later step depends on this source.

## 2. Project configuration

- [x] 2.1 Create `pyproject.toml` with `[build-system]` using `hatchling`.
- [x] 2.2 `[project]` metadata: name `archivey`, version `0.2.0.dev0`,
      `requires-python = ">=3.11"`, description, license, readme.
- [x] 2.3 `[project.optional-dependencies]` runtime extras:
      `7z` (py7zr), `rar` (rarfile), `iso` (pycdlib), `zstd` (zstandard),
      `lz4` (lz4), `cli` (tqdm), and `all` (aggregates every runtime extra).
- [x] 2.4 `[dependency-groups]` `dev` (PEP 735): pytest, mypy, ruff, coverage,
      plus any libs needed to generate test archives.
- [x] 2.5 Tool config: `[tool.mypy]` `strict = true`, `python_version = "3.11"`;
      `[tool.ruff]`; `[tool.coverage]`.
- [x] 2.6 Add `.python-version` (`3.11`) and any `[tool.uv]` settings needed.
- [x] 2.7 Generate and commit `uv.lock` (`uv lock`).

## 3. Port source from DEV

- [x] 3.1 Copy all `src/archivey/*.py` from the DEV checkout (step 1) verbatim
      into this project's `src/archivey/`.
- [x] 3.2 Add `src/archivey/py.typed` (PEP 561 marker) if not present.
- [x] 3.3 `uv sync` and confirm the package imports
      (`uv run python -c "import archivey"`).

## 4. Port test infrastructure

- [x] 4.1 Copy `sample_archives.py`, `create_archives.py`, `conftest.py`,
      `testing_utils.py`, and all `test_*.py` from the DEV checkout.
- [x] 4.2 Move committed binary test archives into `tests/fixtures/`.
- [x] 4.3 Drop or quarantine (with a clear `skip` reason) tests that depend on
...
- [ ] 5.4 Packaging sanity (per `packaging-and-extras` spec):
      ...
      succeed.
- [x] 5.5 `archivey.__version__` resolves via `importlib.metadata`.
- [x] 5.6 Confirm no new public API is exposed yet (private fork baseline).
