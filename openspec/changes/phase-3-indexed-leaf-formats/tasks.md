...
- [ ] 2.5 **(S2)** Brotli **content probe** (`PROBABLE`); skipped when the Brotli
      ...
      position.
- [ ] 2.6 **(S4)** ISO extended 32 774-byte peek; too-short stream → "not ISO", fall
...
- [ ] 7.2 `CostReceipt` values verified per format (ZIP `INDEXED`/`DIRECT`/`SEEKABLE`;
      TAR `INDEXED`/`DIRECT`; single-file `INDEXED`/`SOLID`→lowered; ISO
      `INDEXED`/`DIRECT`/`SEEKABLE`).
...
- [ ] 8.2 **(S2)** `format-single-file-compressors` scenarios for this-phase codecs
      (name inference, one member, gzip `raw_filename`, per-format size rules, cost,
