...
- [ ] 1.1 Catalog every archive shape in `tests/_dev_oracle/sample_archives.py` (and the
      ...
      commit hash.
- [ ] 1.2 Diff the catalog against v2's `tests/sample_archives.py`; produce the ported
...
- [ ] 2.1 Extend v2's corpus dataclasses/builders as needed (expected-error entries,
      per-entry required-dependency markers, format-implemented guard) without breaking
      existing corpus consumers.
- [ ] 2.2 Port the missing shapes for ZIP, TAR (+ compressed variants), single-file
...
- [ ] 2.3 Add the 7z/RAR shapes as inactive entries behind the registry-driven guard.

## 3. Conformance sweep

- [ ] 3.1 `tests/test_corpus_sweep.py`: one parametrized driver — open, compare listing
...
- [ ] 3.2 Run the sweep across the three dependency legs; convert any backend bugs it
      surfaces into their own tracked changes (targeted xfail with a reference, not a fix
      here).

## 4. Oracle deletion

- [ ] 4.1 Delete `tests/_dev_oracle/` (both copies) and remove the `norecursedirs`,
      ruff, and type-checker exclusion entries referencing it.
- [ ] 4.2 Full gates: pytest (three legs), ruff, pyrefly, ty; `openspec validate --all`.

## 5. Documentation sync

- [ ] 5.1 Rewrite PLAN.md's test-strategy step 2/4 (frozen-oracle narrative and the
      Phase 10 deletion task) to the corpus-sweep model; trim the Phase 10 task list
      accordingly.
- [ ] 5.2 Update CONTRIBUTING.md (tool-exclusion note) and CLAUDE.md's `archivey-dev`
      section (the DEV repo stays the external reference; in-repo clone is gone).
- [ ] 5.3 Sync the `testing-contract` delta into `openspec/specs/` and archive this
      change.
