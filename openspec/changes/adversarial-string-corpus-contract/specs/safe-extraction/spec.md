...
destination filesystem at write time — a UTF-8-enforcing filesystem (APFS, some network
filesystems) rejects the byte sequence with `EILSEQ`/`EINVAL`. Extraction SHALL translate
that refusal into a typed `ExtractionError` (carrying the member name and the original
`OSError` as cause) rather than letting the raw `OSError` escape. Under
`OnError.CONTINUE` it is an ordinary per-member failure result. On filesystems that
accept arbitrary bytes (typical Linux), the member extracts normally; the refusal is an
environment outcome, not a property of the archive.
...
- **THEN** extraction raises a typed `ExtractionError` whose cause is the filesystem's
  `OSError` (never a raw `OSError`), or records a failure result under
  `OnError.CONTINUE`
