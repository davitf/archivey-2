...
- [x] 0.1 **Commit the interface invariant now; defer the feature** — lock `_open_member`
      reentrancy before Phase 6 writes new backends; parallel extraction stays a future change.
- [x] 0.2 **Target regime is random-access, member-list-materialized** — streaming/forward-pass
      is inherently sequential and out of scope.
- [x] 0.3 **Decide the feature with numbers** — no parallel-extraction implementation until the
...
- [x] 1.1 Add the `archive-reading` delta (this change's spec) — random-access backend
...
- [x] 1.2 **Audit existing backends** (`directory`, `zip`, `tar`, `iso`, `single_file`)
...
- [x] 1.4 Make the invariant discoverable by a Phase 6 implementer: docstring on the ABC
...
- [x] 2.1 **Member-cache one-time-build safety** — sketch the safe options (init-under-lock vs.
...
- [x] 2.5 **"N readers over one path" vs. "one reader + shared source"** — weigh the two
...
- [x] 2.6 **Recommendation** — if the analysis finds the ABC needs more than the §1 invariant
      to avoid a retrofit, state exactly what, so it can be decided before Phase 6 locks the ABC.
      → **No further ABC change needed before Phase 6.**

## 3. Outcome

- [x] 3.1 `openspec validate --strict parallel-reader-exploration` passes.
- [x] 3.2 The written analysis (§2) is captured (proposal + a docs note) as the seed for a
      future `parallel-extraction` change; that feature is explicitly **not** implemented here.
- [x] 3.3 Pyrefly + ty + ruff clean (only if any audit-driven tightening lands).
