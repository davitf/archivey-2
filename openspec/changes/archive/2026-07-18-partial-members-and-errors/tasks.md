...
- [x] 1.1 Add frozen `MemberListReport` (`members`, `error`, `diagnostics`) with
      sequence ergonomics mirroring `ExtractionReport`; export from public surface
- [x] 1.2 Add `ArchiveReader.members_report() -> MemberListReport` to the ABC and
      `BaseArchiveReader` skeleton
- [x] 1.3 Rename `get_members_if_available` → `members_report_if_available`
...
- [x] 2.1 Store materialization as a single `MemberListReport` (completeness is
...
- [x] 2.2 Identity-stamp recovered members (`member in reader`); allow
      `open(member)` by identity; derive `members()` / `scan_members()` /
      `get(name)` from the stored report (raise `error` if set) so they stay
      complete-or-raise
- [x] 2.3 Implement `members_report()` (return the stored report; replay on
...
- [x] 3.1 Change RA `__iter__` / `stream_members` so terminal archive-level
      listing errors yield the recovered prefix then raise (match streaming)
- [x] 3.2 Confirm RA `extract_all` extract-prep remains fail-closed (no partial
      writes) on those errors
- [x] 3.3 Confirm streaming `__iter__` / `stream_members` / `members_report` /
      `scan_members` contracts match the delta specs (report vs raise)

## 4. TAR / Option F first consumer

- [x] 4.1 Wire TAR rejected-header / strict absent-short trailer paths through
      the shared incomplete + report + yield-then-raise machinery
- [x] 4.2 Add/adjust unit tests for TAR RA and streaming: `members_report()`
      fields, `members()` raise, `__iter__` yield-then-raise, no complete cache
      publish, `open` on recovered member

## 5. CLI and docs

- [x] 5.1 Switch CLI `list` to `members_report()`: print recovered members; stderr
      + exit `1` when `error` is set
- [x] 5.2 Document dual listing contract in `docs/usage.md` / Gotchas / API notes
      per documentation delta
- [x] 5.3 Record Q7 decision in `review/api-coherence/QUESTIONS.md` and
      cross-link `review/backlog.md` / `STATUS.md` to this change

## 6. Verify

- [x] 6.1 Targeted pytest for report / yield-then-raise / CLI list honesty
      (TAR fixtures); three-config not required until apply lands on main path
- [x] 6.2 `openspec validate --strict partial-members-and-errors`
- [x] 6.3 `uv run --no-sync ruff format` / `ruff check` / pyrefly + ty clean on
      touched Python
