...
- When multiple distinct static candidates exist, or a provider may supply retries, the
  ...
  - **STORED members**: one shared pass over the ciphertext computes every surviving
    ...
    the central-directory CRC wins. At most one extra full read total, not one per
    candidate. (A compressibility early-accept probe was investigated and dropped —
    STORED members are typically already-compressed media that look random to such
    heuristics.)
