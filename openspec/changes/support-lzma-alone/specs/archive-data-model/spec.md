...
`UNCOMPRESSED`, `GZIP`, `BZIP2`, `XZ`, `ZSTD`, `LZ4`, `LZIP`, `LZMA`, `ZLIB`,
`BROTLI`, and `UNIX_COMPRESS`. Standalone raw-stream constants SHALL exist for
`GZ`, `BZ2`, `XZ`, `ZST`, `LZ4`, `LZIP`, `LZMA`, `ZLIB`, `BROTLI`, and `Z`.
Uncommon container-codec pairs such as `tar.lz` and `tar.lzma` SHALL be
constructed on demand as `ArchiveFormat(container, stream)` rather than receiving
named constants. `file_extension()` SHALL derive from the stream for raw streams
and from `container.codec` for containers.

`StreamFormat.LZMA` (`"lzma"`) SHALL name the legacy LZMA Alone stream
(`lzma.FORMAT_ALONE`), not raw 7z/ZIP `FORMAT_RAW` LZMA1/LZMA2.
...
| Open a `.lzma` Alone stream | `ArchiveFormat.LZMA`; container `RAW_STREAM`; stream `LZMA` |
| Open `tar.lzma` | Equal to `ArchiveFormat(ContainerFormat.TAR, StreamFormat.LZMA)`; `file_extension() == "tar.lzma"` |
