...
**What it does:** Adds members_report returning a MemberListReport that always carries recovered members, an optional terminal error, and diagnostics. Keeps members and scan_members as complete-or-raise. Aligns random-access iteration and stream_members with streaming: yield the prefix, then raise. Never publishes a partial list as a successful complete cache. CLI list prints the prefix and exits one when the report carries an error. Random-access extract prep stays fail-closed.
...
**Your call later:** Whether MemberListReport should iterate like ExtractionReport (design leans yes); re-scan versus replay when members_report is called again after an incomplete random-access pass on a seekable source.
