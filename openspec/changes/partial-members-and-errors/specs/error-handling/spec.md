...
| API | Contract |
| --- | --- |
| `list_members()` | Always returns `MemberListReport` with prefix in `members` and the failure in `error` |
...
| Case | Expected |
| --- | --- |
| TAR rejected header after 3 members; `list_members()` | `len(members)==3`, `error` is `CorruptionError` |
