...
- `list_members()` → `MemberListReport` — recovered members plus `error` when the
...
| Case | Expected |
| --- | --- |
| Reader wants inventory of a possibly damaged tar | Finds `list_members()` recipe (check `error`, use `members`) |
| Reader wants “fail if not complete” | Directed to `members()` / `scan_members()` |
| Reader looks for salvage/best-effort | Pointed to reserved/future salvage — not `list_members` |
