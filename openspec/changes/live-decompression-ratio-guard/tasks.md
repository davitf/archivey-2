...
- [x] 1.1 Add a thin counting reader that wraps the raw compressed source and increments a
...
- [x] 1.2 Surface the archive's outer running total on the reader as
...
- [x] 2.1 Give `BombTracker` a consumed-bytes sampler (a zero-arg callable returning
...
- [x] 2.2 The coordinator passes the reader's `compressed_bytes_consumed` sampler into the
      tracker once per extraction call.

## 3. Tests

- [x] 3.1 Streaming `.tar.gz` zip-bomb from a non-seekable pipe is caught by the **live** ratio
      before the absolute `max_extracted_bytes` cap.
- [x] 3.2 Plain (uncompressed) `.tar` from a pipe never trips the live ratio; cumulative byte
      cap still applies.
- [x] 3.3 A size-probeable `.tar.gz` uses the **static** archive-wide ratio (live path not
      engaged; ratio not double-counted).
- [x] 3.4 The live ratio halts even under `OnError.CONTINUE`.

## 4. Gates

- [x] 4.1 `uv run pyrefly check` + `uv run ty check` + `uv run ruff` clean.
- [x] 4.2 Full suite green; new streaming-bomb scenarios pass.
