# Internal reference

Maintainer / contributor material. Not the primary user guide.

| Doc | Role |
| --- | --- |
| [Threat model](threat-model.md) | Trust boundaries, enforced guarantees, open security/compat gaps |
| [Compression-library analysis](library-analysis.md) | Per-codec backend choice and rationale |
| [Known issues](known-issues.md) | Accelerator lifecycle / macOS coexistence notes |
| [RAR ↔ unrar piping investigation](rar-unrar-piping-investigation.md) | Why `unrar` can't stream a piped archive; temp-file vs memfd vs synthetic-member trade-offs; solid-demux fragility |

Normative behavior remains in `openspec/specs/`. Product framing: `VISION.md` at the
repository root. Decision summaries: [decision log](../decisions/index.md).
