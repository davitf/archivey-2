...
| F6 | **Medium** | `verify.py` (`VerifyingStream`) | A **hashed** member that decodes to **more than its stated size is not capped** at the declared size — `VerifyingStream` has no `size`, so it reads/delivers the whole over-long output and only raises a digest mismatch at EOF (and *nothing* if the attacker matched the CRC to the bloated content: verified — 200 KB delivered for a declared 10 KB, no error). The size cap lives only in #113's `LengthVerifyingStream`, which is **skipped for hashed members**. Only the extract-time `ExtractionLimits` backstops it — not a raw `open(m).read()`. Surfaced while reviewing #113's size-verification design for F2. VISION #2/#4. |
