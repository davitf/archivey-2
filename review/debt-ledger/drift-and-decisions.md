...
|---|---|---|---|
| **DD1** | Performance **Q2** — where the wall budget is enforced (nightly-vs-previous JSON, 2× band on read_all, or informational) | `review/performance/QUESTIONS.md` Q2 | **IMPLEMENTED (2026-07-24, #171)** — nightly wall-ratio drift gate live (`benchmark-wall.yml --wall-drift-baseline`); absolute bands informational. |
...
| DD4 | `rapidgzip-truncation-investigation` (**1/13** as of 2026-07-24) — the shipped ISIZE backstop is a heuristic built on admittedly incomplete knowledge (`proposal.md`) | in-flight change | **DECIDED (2026-07-20)** — **PAY before 0.2.0** (debt-ledger Q4). Enrichment in change `design.md`; still deferred. Related gzip-family truncation work landed via `gzip-zlib-truncation-recovery` (#183, N2) but does **not** close DD4's rapidgzip-accelerator path. |
| DD13 | pyppmd native **UAF** (PPMd `[7z]`) — mitigated in-tree (#188/#189), upstream unfixed, CI keeps `--allow-exit-after-green` | `docs/internal/known-issues.md:171`; `ppmd-native-investigation-results.md` §J | **KEEP for 0.2.0** (mitigated, opt-in path) — **file the upstream report**; revisit dropping the CI flag once the deterministic valgrind gate runs on the hot-race platforms. |
| DD14 | Two ✓ Complete OpenSpec changes unarchived (`unify-pass-driver`, `gzip-zlib-truncation-recovery`) | `openspec list` | **PAY (2026-07-24)** — `openspec archive` sweep after syncing main specs (same shape as D5/D6). |
